...
| 2 | S24 | `Android-AP1` / `***REDACTED***` (mDNS `spartanhub.local`) | Gateway | **nur manuell** |
...
Lokale Zugangsdaten bleiben in `src/wifi_secret.h` und werden **nicht** committed. Vorlage: `src/wifi_secret.example.h`. `wifi.txt` (Geraete-Profile) ist ebenfalls gitignored. Ablage-/Einspiel-Konzept (privates Repo): siehe [`SECRETS-POLICY.md`](SECRETS-POLICY.md).
